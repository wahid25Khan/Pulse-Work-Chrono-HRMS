...
export default class PwchronoExpenseClaimAdmin extends LightningElement {
  ...
  loadClaims() {
    ...
    call
      ...
      .catch((err) => {
        this.error =
          err?.body?.message || err?.message || "Error loading team claims.";
      })
      .finally(() => {
  ...
  get isPendingView() {
    return this.activeView === VIEW_PENDING;
  }
  get isAllView() {
    return this.activeView === VIEW_ALL;
  }

  get pendingTabClass() {
    return `nav-link${this.activeView === VIEW_PENDING ? " active fw-medium" : ""}`;
  }
  get allTabClass() {
    return `nav-link${this.activeView === VIEW_ALL ? " active fw-medium" : ""}`;
  }
