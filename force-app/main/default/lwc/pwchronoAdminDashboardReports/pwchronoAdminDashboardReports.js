...
export default class PwchronoAdminDashboardReports extends LightningElement {
    ...
    @wire(getDashboardSummary, { portalUserId: '$employeeId', sessionToken: '$sessionToken' })
    ...
    get leaveByType() {
        ...
        return types.map((item, index) => ({
            ...
            percentage: this.leaveTotal > 0 ? Math.round((item.count / this.leaveTotal) * 100) : 0
        }));
    }
    ...
    get employeesByDepartment() {
        ...
        const colors = ['bg-indigo-500', 'bg-teal-500', 'bg-orange-500', 'bg-pink-500', 'bg-cyan-500', 'bg-lime-500', 'bg-amber-500', 'bg-rose-500'];
    ...
    get expenseTotalAmount() {
        return this.formatCurrency(this.dashboardData?.expenseMetrics?.totalAmount || 0);
    }

    get expenseApprovedAmount() {
        return this.formatCurrency(this.dashboardData?.expenseMetrics?.approvedAmount || 0);
    }

    get expensePendingAmount() {
        return this.formatCurrency(this.dashboardData?.expenseMetrics?.pendingAmount || 0);
    }
    ...
    handleRefresh() {
        this.isLoading = true;
        getDashboardSummary({ portalUserId: this.employeeId, sessionToken: this.sessionToken })
            .then(data => {
                this.dashboardData = data;
                this.error = '';
            })
    ...
    renderedCallback() {
        if (this.hasAttendanceData) {
            const attendanceBar = this.template.querySelector('[data-id="attendance-bar"]');
            if (attendanceBar) {
