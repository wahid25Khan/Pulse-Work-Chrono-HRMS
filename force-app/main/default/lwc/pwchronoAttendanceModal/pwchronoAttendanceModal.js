...
export default class PwchronoAttendanceModal extends LightningElement {
    ...
    get isPresent() { return this.status === 'Present'; }
    get isAbsent() { return this.status === 'Absent'; }
    get isLate() { return this.status === 'Late'; }
    get isOnLeave() { return this.status === 'On Leave'; }
    ...
    async handleSave() {
        ...
        try {
            ...
            this.dispatchEvent(new ShowToastEvent({
                title: 'Error',
                message: 'Error saving attendance: ' + (error.body ? error.body.message : error.message),
                variant: 'error'
            }));
        }
    }
