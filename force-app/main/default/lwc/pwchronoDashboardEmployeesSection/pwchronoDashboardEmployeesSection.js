...
export default class PwchronoDashboardEmployeesSection extends LightningElement {
    ...
    get employeeTrendValueClass() {
        return this.employeeTrendIsPositive ? 'text-success fw-bold' : 'text-danger fw-bold';
    }

    get departmentData() {
        if (!Array.isArray(this.employeesByDepartment) || this.employeesByDepartment.length === 0) {
            return [];
        }

        const maxCount = Math.max(...this.employeesByDepartment.map((d) => d.value || 0), 1);
        return this.employeesByDepartment.map((dept) => {
            const percent = Math.max(0, Math.min(100, Math.round(((dept.value || 0) / maxCount) * 100)));
