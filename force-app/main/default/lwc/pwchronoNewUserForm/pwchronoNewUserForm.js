...
export default class PwchronoNewUserForm extends LightningElement {
    ...
    async loadFeatures() {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                this.featureLoadError = 'No features are available in the system. Please contact your administrator.';
                this.features = [];
            }
        } catch (error) {
            // Capture specific error message from Apex
            const errorMsg = error?.body?.message || error?.message || 'Failed to load features. Please try again.';
    ...
    async handleSave() {
        ...
        this.isSaving = true;
        const activeFeatures = this.features.filter(f => f.hasAccess).map(f => f.name);
        ...
        try {
            ...
            this.showToast('Success', `User ${this.newUser.Name} created successfully.`, 'success');
